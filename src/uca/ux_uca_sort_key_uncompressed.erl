...
sort_key(C=#uca_options{strength=MaxLvl, backwards=B}, S) ->
    ...
    R = [], % Remains
    K = [], % Key
    lists:reverse(do_sort_key1(MaxLvl, B, W, A, R, K)).
...
do_sort_key1(S, B, [WH|WT], A, R, K) ->
    {NewA, Ints} = do_alt(A, WH),
    case Ints of
    [0|Rem] ->
        do_sort_key1(S, B, WT, NewA, [Rem|R], K);
    [L1|Rem] ->
        do_sort_key1(S, B, WT, NewA, [Rem|R], [L1|K]);
    _ ->
