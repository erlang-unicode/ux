...
freq_dict(V) -> uxunidata:freq_dict(V).
nfc_qc(V) -> uxunidata:nfc_qc(V).
nfd_qc(V) -> uxunidata:nfd_qc(V).
nfkc_qc(V) -> uxunidata:nfkc_qc(V).
nfkd_qc(V) -> uxunidata:nfkd_qc(V).
is_comp_excl(V) -> uxunidata:is_comp_excl(V).
is_compat(V) -> uxunidata:is_compat(V).
comp(V1, V2) -> uxunidata:comp(V1, V2).
decomp(V) -> uxunidata:decomp(V).
ducet_r(V) -> uxunidata:ducet_r(V).
ducet(V) -> lists:reverse(ducet_r(V)).

types(Str) -> lists:map({uxchar, type}, Str).
...
delete_types(Types, Str) -> 
    lists:filter(fun(El) -> 
            not lists:member(uxchar:type(El), Types) 
        end, Str).
...
filter_types(Types, Str) -> 
    lists:filter(fun(El) -> 
            lists:member(uxchar:type(El), Types) 
        end, Str).
...
get_types(Types, [Char|Tail], 
    ...
    FalseStep) -> 
    case apply(Fun, [uxchar:type(Char), Types]) of
...
explode_types_cycle(Types, [Char|Str], Buf, Res) -> 
    case lists:member(uxchar:type(Char), Types) of
...
get_recursive_decomposition(false, Str) -> 
    get_recursive_decomposition(fun uxutils:is_always_false/1, Str, []);
...
%% @private
to_ncr([Char|Tail], Res) -> to_ncr(Tail, uxchar:to_ncr(Char) ++ Res);
to_ncr([         ], Res) -> Res.
...
info_comment(Obj = #unistr_info{str=Str}) ->
    Obj#unistr_info{
        comment=lists:map({uxchar, comment}, Str)
    }.
...
info_col_sort_array_non_ignorable(Obj = #unistr_info{str=Str}) ->
    Obj#unistr_info{ 
        col_sort_array_non_ignorable=uxcol:sort_array_non_ignorable(Str)
    }.

%% @private
info_col_sort_array_blanked(Obj = #unistr_info{str=Str}) ->
    Obj#unistr_info{ 
        col_sort_array_blanked=uxcol:sort_array_blanked(Str)
    }.

%% @private
info_col_sort_array_shifted(Obj = #unistr_info{str=Str}) ->
    Obj#unistr_info{ 
        col_sort_array_shifted=uxcol:sort_array_shifted(Str)
    }.

%% @private
info_col_sort_array_shift_trimmed(Obj = #unistr_info{str=Str}) ->
    Obj#unistr_info{ 
        col_sort_array_shift_trimmed=uxcol:sort_array_shift_trimmed(Str)
    }.

%% @private
info_char_block(Obj = #unistr_info{str=Str}) ->
    Obj#unistr_info{ 
        blocks=lists:map(fun uxchar:block/1, [Ch || Ch <- Str])
    }.
